...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.post('https://sasa-back-vercel.vercel.app/api/get-profile-data', { user: user }, {
          headers: {
  ...
  const save_changes=async(e)=>{
    ...
    try {
      const response = await axios.post('https://sasa-back-vercel.vercel.app/api/update-profile', update_fields, {
        headers: {
          Authorization: localStorage.getItem('token'),
        }
      });
