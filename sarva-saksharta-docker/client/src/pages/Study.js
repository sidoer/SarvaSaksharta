...
const Give_Task = ({ set_show_task }) => {
  ...
  const assign_task = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('http://localhost:5500/api/tasks', task_details, {
        headers: {
          Authorization: localStorage.getItem('token'),
        },
      });
...
const Study = () => {
  ...
  const sendDelete=async(entry)=>{
    ...
    try {
      const response = await axios.post('http://localhost:5500/api/delete-content',{filename:entry.filename},{
        headers: {
          Authorization: localStorage.getItem('token'),
        },
      });
